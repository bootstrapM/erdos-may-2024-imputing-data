...
## Motivation: 
Stock exchanges such as NYSE and NASDAQ average about 252 [trading days](https://en.wikipedia.org/wiki/Trading_day) a year. Markets at such exchanges have trading hours and non-trading hours nationally and internationally. Typically, trading begins at 9:30 AM and ends at 4:00 PM. The remaining hours is what we might call missing data (during which the `Close` price on the $i$'th day evolves to (usually different) `Open` price on $i+1$th day as consequence of, for instance, developments in financial markets worldwide). Analyzing imputation for such time series could therefore yeild insight on international market correlations and the relevent models and market predictors to use for the more practical problem of making forecast in price movements. Another situation of missing data that one could speculate is where an external shock leads to huge jumps in the prices which might temporarily halt trading.
